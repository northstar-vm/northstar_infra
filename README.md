...
The portal home page shows simple CPU/RAM/root disk bars plus a read-only Minecraft panel with player count, sampled player names, and recent redacted log lines. The status API is served by an internal Docker container and is not published directly to the internet.
